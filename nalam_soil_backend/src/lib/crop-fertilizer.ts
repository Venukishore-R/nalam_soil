...
import {
  convertPerAcreToUnit,
  convertPerHectareToPerAcre,
  UNIT_CONVERSION,
} from "./crop-data";
import { SensorRecord } from "./sensor-store";

interface SoilFormula {
  intercepts: {
    nitrogen: number;
    phosphorous: number;
    potassium: number;
...
type CropSoilFormulas = Record<string, SoilFormula>;

const SORGHUM_SOIL_FORMULAS: CropSoilFormulas = {
  red_sandy_loam: {
    intercepts: {
      nitrogen: 4.86 * 50,
      phosphorous: 1.63 * 50,
...
const SUGARCANE_SOIL_FORMULAS: CropSoilFormulas = {
...
const CROP_SOIL_FORMULAS: Partial<Record<CropType, CropSoilFormulas>> = {
  Sorghum: SORGHUM_SOIL_FORMULAS,
  Sugarcane: SUGARCANE_SOIL_FORMULAS,
};

const SOIL_KEY_KEYWORDS: Partial<Record<CropType, Record<string, string[]>>> = {
  Sorghum: {
...
const DEFAULT_SOIL_KEY: Partial<Record<CropType, string>> = {
  Sorghum: "red_sandy_loam",
  Sugarcane: "mixed_black_calcareous",
};

export const SUPPORTED_FERTILIZER_CROPS: CropType[] = [
  "Sorghum",
  "Sugarcane",
  "Paddy",
  "Radish",
...
type PaddyVarietyKey = keyof typeof PADDY_VARIETY_TARGETS;

const PADDY_VARIETY_KEY_MAP: Record<string, PaddyVarietyKey> = {
...
const PADDY_VARIETY_FORMULAS: Record<PaddyVarietyKey, SoilFormula> = {
...
function resolvePaddyVarietyKey(variety?: string | null): PaddyVarietyKey {
...
type RadishStageKey = keyof typeof RADISH_STAGE_TARGETS;

const RADISH_STAGE_KEY_MAP: Record<string, RadishStageKey> = {
  basal: "basal",
...
function resolveRadishStageKey(stage?: string | null): RadishStageKey {
...
function buildStageFormula(
  target: { nitrogen: number; phosphorous: number; potassium: number },
  sensor: SensorRecord,
): SoilFormula {
  return {
    intercepts: {
...
type TapiocaStageKey = keyof typeof TAPIOCA_STAGE_TARGETS;

const TAPIOCA_STAGE_KEY_MAP: Record<string, TapiocaStageKey> = {
  basal: "basal",
...
function resolveTapiocaStageKey(stage?: string | null): TapiocaStageKey {
  const normalized = (stage ?? "").trim().toLowerCase();
  if (!normalized) {
...
const AREA_LABELS: Record<UnitType, string> = {
  acre: "acre",
  hectare: "ha",
  square_meter: "m²",
};

interface BaseFertilizerConfig {
  label: string;
  rate: number;
  rateUnit: "acre" | "hectare";
  massUnit: "tons" | "kg";
  crops?: CropType[];
  stage?: string;
}

const BASE_FERTILIZERS: BaseFertilizerConfig[] = [
  {
...
const CROP_SPECIFIC_BASE_FERTILIZERS: Partial<
  Record<CropType, BaseFertilizerConfig[]>
> = {
  Paddy: [
    {
      label: "Zinc Sulphate",
...
function matchesKeywords(normalized: string, keywords: string[]) {
  if (!normalized) {
    return false;
  }
...
function resolveSoilKey(cropType: CropType, soilType?: string | null): string {
  const formulas = CROP_SOIL_FORMULAS[cropType];
  const defaultKey =
    DEFAULT_SOIL_KEY[cropType] ??
...
function computeNutrientTargets(sensor: SensorRecord, formula: SoilFormula) {
  const nitrogen =
    formula.intercepts.nitrogen -
    formula.coefficients.nitrogen.nitrogen * sensor.nitrogen -
    formula.coefficients.nitrogen.phosphorous * sensor.phosphorous;
  const phosphorous =
    formula.intercepts.phosphorous -
...
interface FertilizerValue {
  value: number;
  unit: string;
}

interface BaseFertilizerOutput {
  label: string;
  perUnitValue: number;
  perUnitLabel: string;
  totalValue: number;
...
export interface FinalRecommendationPayload {
  ...
  baseFertilizers: BaseFertilizerOutput[];
}

function formatPerUnitLabel(massUnit: string, unit: UnitType) {
  return `${massUnit}/${AREA_LABELS[unit]}`;
}

export function calculateCropRecommendation(params: {
  cropType: CropType;
  sensor: SensorRecord;
  soilType: string | null;
  variety?: string | null;
  ...
  let soilKey = "";
  let resolvedVariety: string | null = params.variety ?? null;
  let resolvedStage: string | null = params.growthStage ?? null;
  let selectedFormula: SoilFormula | null = null;
  let nutrientTargets: {
    nitrogen: number;
    phosphorous: number;
    potassium: number;
  ...
  if (isPaddy) {
    ...
    selectedFormula = buildStageFormula(
      RADISH_STAGE_TARGETS[stageKey],
      params.sensor,
    );
    nutrientTargets = computeNutrientTargets(params.sensor, selectedFormula);
  } else if (isTapioca) {
    ...
    resolvedStage = params.growthStage ?? null;
    selectedFormula = buildStageFormula(
      TAPIOCA_STAGE_TARGETS[stageKey],
      params.sensor,
    );
    nutrientTargets = computeNutrientTargets(params.sensor, selectedFormula);
  } else {
  ...
  const perAcreNutrients = isPaddy || isRadish || isTapioca
    ? nutrientTargets
    : {
        nitrogen: convertPerHectareToPerAcre(nutrientTargets.nitrogen),
        phosphorous: convertPerHectareToPerAcre(nutrientTargets.phosphorous),
        potassium: convertPerHectareToPerAcre(nutrientTargets.potassium),
      };
  ...
  const perUserUnit = {
    urea: convertPerAcreToUnit(perAcreFertilizers.urea, params.unit),
    ssp: convertPerAcreToUnit(perAcreFertilizers.ssp, params.unit),
    mop: convertPerAcreToUnit(perAcreFertilizers.mop, params.unit),
  };
  ...
  const stageMatches = (entry: BaseFertilizerConfig) =>
    !entry.stage || entry.stage.toLowerCase() === normalizedStage;

  const applicableBaseFertilizers = BASE_FERTILIZERS.filter(
    (entry) =>
      (!entry.crops || entry.crops.includes(params.cropType)) &&
      stageMatches(entry),
  );
  const cropSpecificFertilizers =
    (CROP_SPECIFIC_BASE_FERTILIZERS[params.cropType] ?? []).filter(
      stageMatches,
    );
  ...
  const baseFertilizers = fertilizerConfigs.map((entry) => {
    const perAcreRate =
      entry.rateUnit === "acre"
        ? entry.rate
        : entry.rate / UNIT_CONVERSION.hectare_to_acre;
    const perUnitValue = convertPerAcreToUnit(perAcreRate, params.unit);
