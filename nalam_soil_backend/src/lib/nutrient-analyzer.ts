...
const NUTRIENT_RANGES: Record<
  CropType,
  {
    nitrogen: [number, number];
    phosphorous: [number, number];
...
function calculateYieldEstimate(
  ...
  const phosphorousScore = Math.min(
    nutrients.phosphorous / ranges.phosphorous[1],
    1,
  );
  const potassiumScore = Math.min(nutrients.potassium / ranges.potassium[1], 1);

  // Average nutrient score
  const avgNutrientScore =
    (nitrogenScore + phosphorousScore + potassiumScore) / 3;
...
export function analyzeNutrients(
  ...
  const phosphorousStatus = getNutrientStatus(
  ...
  const estimatedYield = calculateYieldEstimate(
    nutrients,
    cropType,
    landholdingAcres,
  );
